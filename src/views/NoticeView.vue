...
// 物资清单筛选
const filterCategory = ref<(typeof INVENTORY_CATEGORIES)[number] | '全部'>('全部')
...
const globalFields: FieldDef[] = [
  { key: 'entryCustoms',   label: '入境禁忌 & 海关申报', icon: '🛃', placeholder: '禁止携带物品、申报要求、免税额度...', tapeColor: 'washi-tape--pink',  cardColor: 'trip-card--pink',  stickerColor: 'title-sticker--pink' },
  { key: 'trafficRules',  label: '交通规则（靠左行驶）', icon: '🚗', placeholder: '让行规则、限速、酒驾规定、山路驾驶...', tapeColor: 'washi-tape--mint',  cardColor: 'trip-card--mint',  stickerColor: 'title-sticker--mint' },
  { key: 'paymentNetwork', label: '支付 & 网络',         icon: '💳', placeholder: '信用卡、电话卡、WiFi覆盖...',       tapeColor: 'washi-tape--lemon', cardColor: 'trip-card--lemon', stickerColor: 'title-sticker--lemon' },
  { key: 'emergencyContact', label: '紧急联系电话',      icon: '📞', placeholder: '报警/急救、大使馆、道路救援、旅游投诉...', tapeColor: 'washi-tape--sky', cardColor: 'trip-card--sky',   stickerColor: 'title-sticker--sky' },
  { key: 'campingRules', label: '露营规范',              icon: '⛺', placeholder: 'DOC营地预订、自由露营限制、环保要求...', tapeColor: 'washi-tape--grape', cardColor: 'trip-card--grape', stickerColor: 'title-sticker--grape' },
]
const safetyFields: FieldDef[] = [
  { key: 'hiking',      label: '徒步安全须知', icon: '🥾', placeholder: 'DOC难度、装备要求、路线规划、紧急预案...', tapeColor: 'washi-tape--pink',  cardColor: 'trip-card--pink',  stickerColor: 'title-sticker--pink' },
  { key: 'glacier',     label: '冰川活动须知', icon: '🧊', placeholder: '跟团要求、装备准备、冰缝风险...',       tapeColor: 'washi-tape--sky',   cardColor: 'trip-card--sky',   stickerColor: 'title-sticker--sky' },
  { key: 'tide',        label: '潮汐注意事项', icon: '🌊', placeholder: '潮汐查询、海边安全、涨潮速度...',       tapeColor: 'washi-tape--lemon', cardColor: 'trip-card--lemon', stickerColor: 'title-sticker--lemon' },
  { key: 'wildlife',    label: '野生动物注意', icon: '🦭', placeholder: '海豹/企鹅距离、喂食禁令、沙蝇防护...',   tapeColor: 'washi-tape--mint',  cardColor: 'trip-card--mint',  stickerColor: 'title-sticker--mint' },
  { key: 'temperature', label: '南北岛温差防护', icon: '🌡️', placeholder: '季节温度、洋葱穿衣、山区天气突变...', tapeColor: 'washi-tape--grape', cardColor: 'trip-card--grape', stickerColor: 'title-sticker--grape' },
]
...
const filteredInventory = computed(() => {
  return store.noticeData.inventory.filter((i) => {
    ...
    if (filterOwner.value !== '全部' && i.owner !== filterOwner.value) return false
...
// 物资分类胶带/卡片颜色映射
const inventoryCatColors: Record<string, { tape: string; card: string; sticker: string; emoji: string }> = {
  '证件': { tape: 'washi-tape--pink',  card: 'trip-card--pink',  sticker: 'sticker-badge--pink',  emoji: '📄' },
  '衣物': { tape: 'washi-tape--mint',  card: 'trip-card--mint',  sticker: 'sticker-badge--mint',  emoji: '👕' },
  '电子': { tape: 'washi-tape--sky',   card: 'trip-card--sky',   sticker: 'sticker-badge--sky',   emoji: '📱' },
  '洗漱': { tape: 'washi-tape--lemon', card: 'trip-card--lemon', sticker: 'sticker-badge--lemon', emoji: '🧴' },
  '药品': { tape: 'washi-tape--grape', card: 'trip-card--grape', sticker: 'sticker-badge--grape', emoji: '💊' },
  '户外': { tape: 'washi-tape--pink',  card: 'trip-card--pink',  sticker: 'sticker-badge--peach', emoji: '🏔️' },
  '其他': { tape: 'washi-tape--mint',  card: 'trip-card--mint',  sticker: 'sticker-badge--sky',   emoji: '📦' },
}
...
<template>
  <div class="space-y-5 md:space-y-6" id="notice-view">
    ...
    <!-- ===== Tab 切换：贴纸风 ===== -->
    <div class="trip-card trip-card--lemon p-2 md:p-3 flex gap-1.5 md:gap-2 pt-6 relative flex-wrap">
      ...
      <button
        ...
        :style="{ transform: activeTab === 'global' ? 'rotate(-0.8deg)' : 'rotate(-0.4deg)' }"
      ...
      <button
        ...
        :style="{ transform: activeTab === 'safety' ? 'rotate(0.6deg)' : 'rotate(0.3deg)' }"
      ...
      <button
        ...
        :style="{ transform: activeTab === 'inventory' ? 'rotate(-0.5deg)' : 'rotate(-0.2deg)' }"
      ...
      >
        ...
        >
          <span
            class="sticker-badge !px-1.5 !py-0.5 !text-[9px] font-sticker"
            :class="activeTab === 'inventory' ? '!bg-white !text-grape-600 !border-grape-200' : 'sticker-badge--grape'"
          >
    ...
    >
      <div class="flex items-center gap-2 font-hand text-ink-700 text-sm">
    ...
    <div v-show="activeTab === 'global'" class="space-y-4 md:space-y-5">
      <div
        ...
        :key="f.key"
        :class="[f.cardColor, 'trip-card p-4 md:p-5 pt-8 relative overflow-hidden']"
        :style="{ transform: `rotate(${fIdx % 2 === 0 ? '-0.3' : '0.3'}deg)` }"
      >
        <div :class="['washi-tape', f.tapeColor]" />
        ...
        <div class="flex items-center gap-2.5 mb-4 flex-wrap">
          <span
            :class="['title-sticker text-sm md:text-base', f.stickerColor]"
          >
        ...
        <div class="sticky-note sticky-note--mint">
          <textarea
            :value="(store.noticeData.globalNotices as Record<string, string>)[f.key] || ''"
            :disabled="!store.canEdit"
            rows="5"
            class="textarea-base !bg-white/60 !py-2.5 text-sm"
            :class="{ 'cursor-default !bg-white/80': !store.canEdit }"
            :placeholder="store.canEdit ? f.placeholder : '（主人还没有填写内容哦~）'"
            @blur="(e) => saveGlobal(f.key, (e.target as HTMLTextAreaElement).value)"
          />
        </div>
      </div>
    </div>
    ...
    <div v-show="activeTab === 'safety'" class="space-y-4 md:space-y-5">
      <div
        ...
        :class="[f.cardColor, 'trip-card p-4 md:p-5 pt-8 relative overflow-hidden']"
        :style="{
          transform: `rotate(${fIdx % 2 === 0 ? '0.3' : '-0.3'}deg)`,
          borderLeft: fIdx === 0 ? '6px solid #fb4277' : undefined
        }"
      >
        <div :class="['washi-tape', f.tapeColor]" />
        <span class="doodle-corner top-3 right-4 text-lg animate-sparkle">
          <IconAlert :size="18" class="text-strawberry-500" />
        </span>
        ...
        <div class="flex items-center gap-2.5 mb-4 flex-wrap">
          ...
          <span class="sticker-badge sticker-badge--pink !text-[10px] font-hand animate-wiggle">
            ⚠️ 请务必确认
          </span>
        </div>
        ...
        <div class="sticky-note sticky-note--pink">
          <textarea
            :value="(store.noticeData.safetyTips as Record<string, string>)[f.key] || ''"
            :disabled="!store.canEdit"
            rows="4"
            class="textarea-base !bg-white/60 !py-2.5 text-sm !border-strawberry-200"
            :class="{ 'cursor-default !bg-white/80': !store.canEdit }"
            :placeholder="store.canEdit ? f.placeholder : '（主人还没有填写内容哦~）'"
            @blur="(e) => saveSafety(f.key, (e.target as HTMLTextAreaElement).value)"
          />
        </div>
      </div>
    </div>
    ...
    <!-- ===== 物资清单 ===== -->
    <div v-show="activeTab === 'inventory'" class="space-y-4 md:space-y-5" id="inventory">
      <!-- 进度卡片 -->
      <div class="trip-card trip-card--grape p-5 pt-8 relative">
        ...
        <div class="flex items-center justify-between mb-4 flex-wrap gap-3">
          <div>
            <span class="sticker-badge sticker-badge--grape font-sticker text-sm mb-2 inline-block">
              🎯 准备进度
            </span>
            <div class="font-sticker text-xl md:text-2xl font-extrabold text-ink-800">
              {{ store.packedInventoryCount }} / {{ store.totalInventoryCount }}
              <span class="font-hand text-sm md:text-base text-ink-500 ml-2 font-normal">
                已完成 {{ progressPercent }}%
              </span>
            </div>
            <div
              v-if="progressPercent === 100"
              class="sticker-badge sticker-badge--mint mt-2 animate-wiggle"
            >
              🎉 太棒了！全部准备完成~
            </div>
          </div>
          <div
            class="w-16 h-16 md:w-20 md:h-20 rounded-full flex items-center justify-center text-white font-sticker shadow-sticker border-4 border-white"
            style="background: linear-gradient(135deg, #c084fc 0%, #9333ea 100%); box-shadow: 0 4px 0 #7e22ce, 0 8px 20px rgba(147,51,234,0.35);"
          >
            <span class="text-xl md:text-2xl font-extrabold">{{ progressPercent }}%</span>
          </div>
        </div>
        <!-- 彩虹进度条 -->
        <div class="h-4 bg-lemon-50 rounded-full overflow-hidden border-2 border-lemon-200">
          <div
            ...
            style="background: linear-gradient(90deg, #fb4277 0%, #ffd518 33%, #4bb04a 66%, #a855f7 100%)"
      ...
      <div class="trip-card trip-card--sky p-4 pt-7 space-y-3 relative">
        ...
        <div class="flex flex-wrap gap-2 items-center">
          ...
          >
            ...
            <option v-for="c in INVENTORY_CATEGORIES" :key="c" :value="c">{{ inventoryCatColors[c].emoji }} {{ c }}</option>
          </select>
          <select
            v-model="filterOwner"
          ...
          >
            ...
            <option v-for="o in inventoryOwners" :key="o" :value="o">{{ o === '共用' ? '🤝 共用' : '🧑 ' + o }}</option>
          </select>
          <select
        ...
        <transition name="expand">
          ...
          >
            <div class="grid grid-cols-1 md:grid-cols-12 gap-2 md:gap-3 items-center">
              <input
                v-model="newItemName"
                type="text"
      ...
      <div
        ...
        :key="cat"
        :class="[inventoryCatColors[cat].card, 'trip-card p-0 pt-6 relative overflow-hidden']"
        :style="{ transform: `rotate(${Math.random() > 0.5 ? '-0.2' : '0.2'}deg)` }"
      >
        <div :class="['washi-tape', inventoryCatColors[cat].tape]" />

        <!-- 分类标题 -->
        <div class="flex items-center justify-between px-4 md:px-5 py-3 border-b-2 border-dashed border-ink-100/60">
          <div class="flex items-center gap-2 flex-wrap">
            <span
              :class="['sticker-badge !px-3 !py-1 font-sticker text-sm', inventoryCatColors[cat].sticker]"
            >
              <span class="mr-1">{{ inventoryCatColors[cat].emoji }}</span>{{ cat }}
            </span>
          </div>
          <span class="sticker-badge sticker-badge--lemon !px-2 !py-0.5 !text-[10px] font-sticker">
        ...
        <div class="divide-y divide-dashed divide-ink-100">
          <div
            ...
            :style="{ transform: `rotate(${iIdx % 3 === 0 ? '-0.15' : iIdx % 3 === 1 ? '0.1' : '-0.05'}deg)` }"
          >
            <!-- 勾选框：草莓色圆形贴纸风格 -->
            <button
              type="button"
            ...
            <div class="flex-1 min-w-0">
              <div class="flex items-center gap-2 flex-wrap">
                ...
                <span
                  v-if="item.checked"
                  class="sticker-badge sticker-badge--mint !px-2 !py-0.5 !text-[10px] font-sticker animate-wiggle"
                >
                  ✅ 已备齐
                </span>
              </div>
              <div class="mt-1 flex items-center gap-2 text-[11px] font-hand text-ink-500">
                <span
                  :class="[
                    'sticker-badge !px-2 !py-0.5 !text-[10px] font-sticker',
                    item.owner === '共用' ? 'sticker-badge--grape' : 'sticker-badge--sky'
                  ]"
                >
