...
const cityColorMap: Record<
  string,
  { card: string; badge: string; selected: string }
> = {
  AKL: {
    card: "trip-card--pink",
    badge: "sticker-badge--pink",
    selected: "bg-gradient-to-br from-strawberry-500 to-strawberry-400",
  },
  ZQN: {
    card: "trip-card--grape",
    badge: "sticker-badge--grape",
    selected: "bg-gradient-to-br from-grape-500 to-grape-400",
  },
  CHC: {
    card: "trip-card--lemon",
    badge: "sticker-badge--lemon",
    selected: "bg-gradient-to-br from-lemon-600 to-lemon-500",
  },
  WAN: {
    card: "trip-card--mint",
    badge: "sticker-badge--mint",
    selected: "bg-gradient-to-br from-primary-500 to-primary-400",
  },
  MTC: {
    card: "trip-card--sky",
    badge: "sticker-badge--sky",
    selected: "bg-gradient-to-br from-skyblue-500 to-skyblue-400",
  },
  MIL: {
    card: "trip-card--sky",
    badge: "sticker-badge--sky",
  ...
  WLG: {
    card: "trip-card--pink",
    badge: "sticker-badge--peach",
    selected: "bg-gradient-to-br from-peach-500 to-peach-400",
  },
  ROT: {
    card: "trip-card--lemon",
    badge: "sticker-badge--lemon",
  ...
  <div class="space-y-5 md:space-y-6" id="weather-view">
    ...
    >
      <div class="flex items-start gap-3">
        <IconAlert :size="22" class="text-strawberry-500 mt-0.5 shrink-0" />
        <div>
    ...
    <div class="trip-card trip-card--sky p-4 md:p-5 pt-7 relative">
      <div class="text-xs text-ink-500 mb-3 flex items-center gap-1">
      ...
      <div class="grid grid-cols-4 md:grid-cols-8 gap-2 md:gap-3">
        ...
        >
          <div
            class="font-bold text-xs md:text-sm truncate"
            :class="selectedCity === city.code ? 'text-white' : ''"
          >
            {{ city.name }}
          </div>
          <div class="text-[9px] md:text-[10px] mt-0.5 opacity-75 truncate">
            {{ city.region }}
          </div>
          <!-- 温度小数字 -->
          <div
            v-if="cityWeathers[city.code]?.current"
            class="text-base md:text-lg font-extrabold mt-1.5"
          ...
          >
            <WeatherIcon
              :type="cityWeathers[city.code].current.icon"
              :size="34"
            />
          </div>
          <!-- 选中角标 -->
    ...
    <div
      ...
      class="trip-card trip-card--sky p-6 md:p-8 space-y-4 pt-8 relative"
    >
    ...
    <template v-else-if="current">
      <div
        ...
        :class="{
          '!bg-gradient-to-br from-lemon-50 via-peach-50 to-strawberry-50':
            current.temperature >= 22,
          '!bg-gradient-to-br from-skyblue-50 via-grape-50 to-primary-50':
            current.temperature < 22 && current.temperature >= 12,
          '!bg-gradient-to-br from-grape-50 via-skyblue-50 to-primary-50':
      ...
      >
        ...
        <div
          class="relative flex flex-col md:flex-row gap-5 md:gap-8 items-start md:items-center"
        >
          <!-- 图标 + 大温度 -->
          <div class="flex items-center gap-4 md:gap-6">
            ...
            <div>
              <div class="flex items-start">
                <!-- 温度大数字 -->
                <span
                  class="text-6xl md:text-7xl font-extrabold text-ink-800 leading-none"
                >
                  {{ current.temperature.toFixed(0) }}
                </span>
                <span class="text-2xl md:text-3xl text-ink-500 mt-2">°C</span>
              </div>
              <div class="mt-1 text-sm md:text-base text-ink-600">
                体感 {{ current.feelsLike.toFixed(1) }}°C ·
                {{ current.description }}
              </div>
          ...
          <!-- 详细指标贴纸 -->
          <div
            class="flex-1 w-full grid grid-cols-2 md:grid-cols-6 gap-2 md:gap-3"
          >
            <!-- 体感 -->
            <div class="sticky-note sticky-note--mint !p-3 text-center">
              <div
                class="flex items-center justify-center gap-1 text-ink-500 text-[10px] md:text-xs mb-1"
              >
                <IconThermometer :size="12" /> 体感
              </div>
              <div class="font-extrabold text-ink-800 text-sm md:text-base">
            ...
            <div class="sticky-note sticky-note--pink !p-3 text-center">
              ...
              <div class="font-extrabold text-ink-800 text-sm md:text-base">
                ...
                }}<span class="text-[10px] font-normal text-ink-500 font-sans"
                  >km/h</span
                >
              </div>
              <div class="text-[9px] text-ink-500 mt-0.5">
            ...
            <div class="sticky-note sticky-note--lemon !p-3 text-center">
              ...
              <div class="mt-0.5">
                <span
                  :class="[
                    'sticker-badge !px-1.5 !py-0.5 !text-[8px]',
                    uvLevel(current.uvIndex).color,
                  ]"
                >
            ...
            <!-- 城市 -->
            <div
              class="sticky-note sticky-note--grape !p-3 text-center col-span-2 md:col-span-1"
            >
              <div
                class="flex items-center justify-center gap-1 text-ink-500 text-[10px] md:text-xs mb-1"
              >
              ...
              <div class="font-extrabold text-ink-800 text-sm truncate">
                {{ current.location }}
              </div>
              <div class="text-[9px] text-ink-500 truncate mt-0.5">
                {{
        ...
        <!-- ===== 出行建议：葡萄紫便签 ===== -->
        <div
          class="relative mt-6 pt-5 border-t-2 border-dashed border-skyblue-200/70"
        >
          <div class="flex items-center gap-2 mb-3 flex-wrap">
            <span
              class="title-sticker title-sticker--grape text-sm md:text-base"
            >
              💡 出行建议
            </span>
            <span
              v-if="
                advices.some(
                  (a) =>
                    a.includes('雨') || a.includes('防晒') || a.includes('风'),
                )
              "
              class="sticker-badge sticker-badge--lemon !text-[10px]"
              >要注意哦~</span
            >
          </div>
          ...
          <div class="grid sm:grid-cols-2 gap-3">
            ...
            >
              ...
              <span class="text-sm text-ink-700 leading-relaxed flex-1">{{
                adv
              }}</span>
            </div>
          </div>
      ...
      <div class="trip-card trip-card--mint p-4 md:p-6 pt-8 relative">
        ...
        <div
          class="grid grid-cols-2 sm:grid-cols-5 md:grid-cols-10 gap-2 md:gap-3"
        >
          <div
          ...
          >
            ...
            >
              <span
                class="sticker-badge sticker-badge--pink !px-2 !py-0.5 !text-[9px]"
                >今日</span
              >
            </div>

            <div class="text-[10px] md:text-xs text-ink-600 font-bold">
              {{ i === 0 ? "今天" : formatDateCN(day.date).split(" ")[0] }}
            </div>
            <div class="text-[9px] md:text-[10px] text-ink-400 mt-0.5">
            ...
            <div
              class="text-[9px] md:text-[10px] text-ink-500 h-8 leading-tight overflow-hidden"
            >
              {{ day.description }}
            </div>

            <div
              class="mt-1.5 flex items-center justify-center gap-0.5 font-extrabold"
            >
              <span class="text-strawberry-500 text-sm"
                >{{ day.tempHigh.toFixed(0) }}°</span
              >
              <span class="text-ink-300 text-[10px] mx-0.5">/</span>
              <span class="text-skyblue-600 text-sm"
                >{{ day.tempLow.toFixed(0) }}°</span
              >
            </div>
            ...
            <div class="mt-1 flex items-center justify-center gap-0.5">
              <span
                :class="[
                  'sticker-badge !px-1.5 !py-0.5 !text-[8px] md:!text-[9px]',
      ...
      <div class="trip-card trip-card--lemon p-4 md:p-6 pt-8 relative">
        ...
        <div class="sticky-note sticky-note--pink">
          <div class="flex items-start gap-3 md:gap-4">
            <div
              class="w-14 h-14 md:w-16 md:h-16 rounded-2xl bg-white shadow-sticker flex items-center justify-center text-3xl md:text-4xl shrink-0 border-2 border-white"
            >
              {{
                selectedCity === "AKL"
                  ? "🏙️"
            ...
            <div class="flex-1 min-w-0">
              <div class="flex items-center gap-2 flex-wrap">
                <span class="font-extrabold text-ink-800 text-lg md:text-xl">
              ...
              <div
                class="mt-2 text-sm md:text-base text-ink-700 leading-relaxed"
              >
                {{
                  NZ_CITIES.find((c) => c.code === selectedCity)?.description
                }}
